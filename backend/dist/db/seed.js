...
async function seedDatabase() {
    ...
    // Create users
    const admin = await index_1.default.user.upsert({
    ...
    const operator = await index_1.default.user.upsert({
    ...
    const reviewer = await index_1.default.user.upsert({
    ...
    const UTBK_REFERENCE_CONTEXT = "Penalaran Umum (PU) adalah subtes Tes Potensi Skolastik (TPS) di UTBK SNBT yang mengukur " +
    ...
    const utbkSkills = [
        {
            ...
            systemPrompt: "Kamu adalah pembuat soal Penalaran Induktif untuk subtes Penalaran Umum (PU) UTBK SNBT.\n\n" +
        ...
        },
        {
            ...
            systemPrompt: "Kamu adalah pembuat soal Penalaran Deduktif untuk subtes Penalaran Umum (PU) UTBK SNBT.\n\n" +
        ...
        },
        {
            ...
            systemPrompt: "Kamu adalah pembuat soal Penalaran Kuantitatif untuk subtes Penalaran Umum (PU) UTBK SNBT.\n\n" +
    ...
    for (const skillData of utbkSkills) {
        const existing = await index_1.default.skill.findFirst({
            where: { name: skillData.name },
        });
        if (!existing) {
            const created = await index_1.default.skill.create({
                data: { ...skillData, version: "1.0", createdBy: admin.id },
            });
            console.log("  → UTBK Skill created:", created.name);
        }
        else {
            await index_1.default.skill.update({
