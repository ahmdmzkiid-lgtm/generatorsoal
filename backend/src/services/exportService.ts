...
const KUNCI_FILL: Partial<ExcelJS.Fill> = {
  type: "pattern",
  pattern: "solid",
...
const ALT_FILL: Partial<ExcelJS.Fill> = {
  type: "pattern",
  pattern: "solid",
...
function resolveField(obj: Record<string, unknown>, fieldPath: string): unknown {
  ...
  for (const part of parts) {
    ...
    if (arrayMatch) {
      ...
      current = arr[index];
      if (typeof current === "object" && current !== null && "value" in (current as Record<string, unknown>)) {
...
export interface ExportRow {
  ...
  difficulty: string;
  tags: string[];
  skill?: { name: string; subject: string; gradeLevel: string };
}
...
export async function generateXlsx(
  ...
  rows.forEach((row, idx) => {
    ...
    excelRow.eachCell((cell, colIdx) => {
      ...
      if (isKunci) {
        cell.fill = KUNCI_FILL as ExcelJS.Fill;
      } else if (isAlt) {
  ...
  const colWidths = mappings.map((m) => {
    if (m.excelColumn === "SOAL") return 45;
    if (m.excelColumn.startsWith("OPSI_")) return 32;
    if (m.excelColumn === "KUNCI_JAWABAN") return 14;
    if (m.excelColumn === "PEMBAHASAN") return 55;
    return 20;
  });
  colWidths.forEach((w, i) => {
    try { sheet.getColumn(i + 1).width = w; } catch { /* skip */ }
  });
