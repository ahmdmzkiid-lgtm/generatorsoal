...
export default function BankSoal() {
  ...
  async function handleUnpublish(questionId: string) {
    if (!window.confirm("Yakin ingin meng-unpublish soal ini? Status akan dikembalikan ke antrian review.")) return;
    try {
      setUnpublishingId(questionId);
      await reviewQuestion(questionId, "REJECT", "Unpublished from bank soal");
      setSelectedIds((prev) => {
        const next = new Set(prev);
    ...
    <div className="max-w-7xl mx-auto w-full flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
        <div>
          ...
          <p className="text-sm text-[#434654] mt-1 font-body-md">
            Soal yang sudah dipublish ke Bank Soal. Dapat di-unpublish untuk dikembalikan ke antrian review.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400 bg-white border border-[#c3c6d6] rounded-xl shadow-sm">
          ...
          <p className="text-lg font-semibold text-gray-700">Belum ada soal published</p>
          <p className="text-sm mt-1">
            Soal yang sudah di-approve akan muncul di sini.
          </p>
        </div>
      ) : (
        <>
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-[#f3f3fd] border-b border-[#c3c6d6]">
                    ...
                    <th className="p-4 text-xs font-bold text-[#434654] uppercase tracking-wider w-32">Tgl Publikasi</th>
                    <th className="p-4 text-xs font-bold text-[#434654] uppercase tracking-wider w-28 text-center">Aksi</th>
                  </tr>
                </thead>
                <tbody className="text-sm text-[#191b23] divide-y divide-[#c3c6d6]/50">
                  {data.questions.map((q) => (
                    ...
                    >
                      ...
                      <td className="p-4 align-top">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[#e1f8eb] text-[#005321]">
                          Published
                        </span>
                      </td>
                      <td className="p-4 align-top">
                      ...
                      <td className="p-4 align-top text-center" onClick={(e) => e.stopPropagation()}>
                        <button
                          ...
                          className="border border-[#ba1a1a]/30 text-[#ba1a1a] hover:bg-[#ba1a1a]/5 hover:border-[#ba1a1a] disabled:opacity-50 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors"
                        >
                          {unpublishingId === q.id ? "..." : "Unpublish"}
                        </button>
                      </td>
                    </tr>
                  ))}
